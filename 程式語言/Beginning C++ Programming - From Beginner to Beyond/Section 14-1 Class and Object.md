...
```cpp
int n; cin >> n; Product* products = new Product[n];  // n 是執行時才知道
```
...
### 🔸 範例：
```cpp
Dog* createDog() {     Dog* d = new Dog;     d->name = "Puppy";     return d; }  int main() {     Dog* myDog = createDog();     myDog->bark();   // OK！     delete myDog;    // 不刪會 memory leak }`
```
...
- 初學用 `new/delete` 練習記憶體管理
    
- 後期可學習 smart pointer（例如 `std::unique_ptr`、`std::shared_ptr`）來自動管理記憶體 ✅
